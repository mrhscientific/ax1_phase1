...
\newcommand{\wmark}{\textbf{!}} % Warning mark
...
\midrule
Linear EOS & $P_H = \alpha\rho + \beta\theta + \tau$ & \cmark{} Match \\
Specific heat & $C_v = A_{cv} + B_{cv}\theta$ & \cmark{} Match \\
$\alpha$-eigenvalue & $\alpha = K_{ex}/\ell$ & \cmark{} Match \\
S4 quadrature & 5 angles & \cmark{} Match (when S4 selected) \\
Spherical geometry & Lagrangian shells & \cmark{} Match \\
Lagrangian coordinates & Embedded mesh & \cmark{} Match \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \cmark{} $\alpha$-eigenvalue calculation: $\alpha = K_{ex}/\ell$
    \item \cmark{} Linear equation of state: $P_H = \alpha\rho + \beta\theta + \tau$
    \item \cmark{} Specific heat relation: $C_v = A_{cv} + B_{cv}\theta$
    \item \cmark{} S4 discrete ordinates (when selected)
    \item \cmark{} Spherical Lagrangian geometry
    \item \cmark{} Shell-based spatial discretization
\end{itemize}
...
\begin{itemize}
    \item \cmark{} Core equations verified against 1959 report
    \item \cmark{} S\_n constants appear correct
    \item \wmark{} Unit system requires verification ($\mu$sec, keV, megabars)
    \item \wmark{} Cannot reproduce exact 1959 results due to algorithm changes
\end{itemize}
